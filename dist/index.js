...
var log = function log(_ref) {
  ...
  if ((0, _lodash.has)(console, logLevel)) {
    ...
    logFunc.apply(void 0, [_consoleLogColors.grey.bold(PKG_NAME)].concat(_toConsumableArray(Array.isArray(message) ? message : [message])));
  }
};

var processActionIfPayloadFunction = function processActionIfPayloadFunction(options) {
        ...
        if (typeof actionPayload === 'function') {
          try {
            if (unpackPayloadIfFunction) {
              var resolvedState = (0, _lodash.get)(state, (action === null || action === void 0 ? void 0 : action.target) || '', state);
              var newPayload = actionPayload(resolvedState);

              var resolvedAction = _objectSpread(_objectSpread({}, action), {}, _defineProperty({}, payloadKey, newPayload));
            ...
            log({
              logLevel: logLevel,
              message: "Unable to run payload function, returning original action. MORE INFO: ".concat(e)
            });
            log({
              logLevel: logLevel,
              message: e
...
var isOptionsValid = function isOptionsValid(options) {
  if (!(0, _lodash.isNil)(options.logLevel) && !allowedLogLevel.includes(options.logLevel)) {
    return {
      result: false,
  ...
  if (!(0, _lodash.isNil)(options.payloadKey) && typeof options.payloadKey !== 'string') {
    return {
      result: false,
  ...
  if (!(0, _lodash.isNil)(options.fatal) && typeof options.fatal !== 'boolean') {
    return {
      result: false,
...
var checkDuplicateDispatch = function checkDuplicateDispatch(options) {
  ...
  if (typeof options === 'boolean') {
    ...
    mergedOptions = _objectSpread(_objectSpread({}, DEFAULT_OPTIONS), {}, {
      fatal: options
    });
  } else {
  ...
  return function (store) {
    return function (next) {
      return function (action) {
        // Process action
        var processedAction = processActionIfPayloadFunction(mergedOptions)(action)(store.getState()); // Check that the hash of the previous action matches
        ...
        if (checkHash(processedAction)) {
          ...
          // Handle fatal or not?
          if (fatal) {
            throw new TypeError("A duplicate action has been detected. \n\nAction Unique Hash: ".concat(lastActionHash, "\n\nAction:\n\n").concat(_circularJson["default"].stringify(processedAction, null, 2)));
          } else {
            log({
              logLevel: logLevel,
            ...
            if (typeof action[payloadKey] === 'function' && unpackPayloadIfFunction) {
              console.log(_consoleLogColors.grey.bold(PKG_NAME), 'Unpacked action:', processedAction);
            } // Always show the original action


            console.log(_consoleLogColors.grey.bold(PKG_NAME), 'Original action:', action);
            return next(action);
          }
        }
      };
